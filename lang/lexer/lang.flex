...
%init}
  ...
  Char          = "'" ({Empty} | "(" | ")" | "{" | "}" | "[" | "]" | "=" | ">" | "<" | "!" | "+" | "-" | "*" | "/" | "%" | "&" | "." | ":" | "," | ";" | [:lowercase:] | [:uppercase:] | [:digit:] | \\n | \\t | \\b | \\r | \\\\ | \\' | \\\\') "'"
...
<YYINITIAL>{
    ...
    {Variable}      { return newToken(Terminals.ID, yytext());   }
    {Type}          { return newToken(Terminals.TYPE, yytext());   }
    {Float}         { return newToken(Terminals.FLOATVAR, Float.parseFloat(yytext()));  }
    {Integer}       { return newToken(Terminals.INTEGER, Integer.parseInt(yytext()));  }
    {Char}          { return newToken(Terminals.CHARACTER, yytext());  }
